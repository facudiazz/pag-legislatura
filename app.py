...
def search_file():
    ...
    sort_by_mapping = {
        "date_asc": ("fecha_ingreso", "asc"),
        "date_desc": ("fecha_ingreso", "desc"),
        "name_asc": ("name", "asc"),
        "name_desc": ("name", "desc"),
        "id_asc": ("id", "asc"),
        "id_desc": ("id", "desc")  
    ...
    sort_by, sort_order = sort_by_mapping.get(order, ("id", "desc"))  
    ...
    elif category == "Categoria7":
        query_filters.append(File.category == "Categoria7")

    if start_date:
        try:
            start_date = datetime.strptime(start_date, "%d/%m/%Y").date()
            query_filters.append(File.fecha_ingreso >= start_date.strftime("%Y-%m-%d"))
        except ValueError:
            flash("Formato de fecha de inicio no válido. Utilice el formato dd/mm/yyyy")
            return redirect(url_for("index"))

    if end_date:
        try:
            end_date = datetime.strptime(end_date, "%d/%m/%Y").date()
            query_filters.append(File.fecha_ingreso <= end_date.strftime("%Y-%m-%d"))
        except ValueError:
            flash("Formato de fecha de finalización no válido. Utilice el formato dd/mm/yyyy")
            return redirect(url_for("index"))
    ...
    if autor_coautor:
        query_filters.append(File.autor_coautor.like(f"%{autor_coautor}%"))
    ...
    if sort_by == "fecha_ingreso":
        if sort_order == "desc":
            query = query.order_by(
                func.substr(File.fecha_ingreso, 7, 4).desc(),  
                func.substr(File.fecha_ingreso, 4, 2).desc(),  
                func.substr(File.fecha_ingreso, 1, 2).desc()   
            )
        else:
            query = query.order_by(
                func.substr(File.fecha_ingreso, 7, 4).asc(),   
                func.substr(File.fecha_ingreso, 4, 2).asc(),   
                func.substr(File.fecha_ingreso, 1, 2).asc()    
            )
    elif sort_by == "name":
        if sort_order == "desc":
